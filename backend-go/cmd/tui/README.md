...
- 左侧菜单是页面入口，选中菜单后会进入右侧内容区。
- 右侧下拉组件按按钮处理：必须按 Enter 打开选择遮罩层。
- Esc 按层回退：
	1. 先关闭当前遮罩/弹窗（命令面板、下拉选择、确认框）
	2. 再按一次 Esc，从内容区回到左侧菜单当前项
- 下拉遮罩内只支持 Up/Down 移动，Enter 确认，Esc 取消。

## 交互回归清单

- 页面切换：1-6 可以切页，焦点落在当前页第一个可操作项。
- 侧栏回退：在任意内容区按 Esc，焦点返回左侧菜单。
- 弹层回退：打开命令面板/下拉遮罩后，Esc 仅关闭当前层，不会跳层。
- 下拉行为：Enter 打开，Up/Down 选择，Enter 确认，Esc 取消并回到触发控件。