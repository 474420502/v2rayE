...
说明：服务模式默认不会在启动时强制改写桌面系统代理；如需桌面代理集成，请在配置中把 `systemProxyMode` 改为 `forced_change`，并在 root/systemd 场景下按需设置 `V2RAYN_DESKTOP_USER`。另外，手动指向 `10808/10809` 的本地 HTTP/SOCKS 请求默认仍会遵循当前 `routing.mode`；如果希望“显式本地代理”始终走节点，可把 `localProxyMode` 设为 `force-proxy`。
...
- `localProxyMode`：本地 `10808/10809` HTTP/SOCKS 入口如何处理，默认 `follow-routing`（继续遵循 `routing.mode`）；可设为 `force-proxy`，让显式本地代理流量始终走节点
